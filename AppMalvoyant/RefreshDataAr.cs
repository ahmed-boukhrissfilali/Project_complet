...
{
    public class RefreshDataAr
    {
         public void RefreshDatar()

        {
            ...
            var response = httpClient.GetAsync("https://www.hespress.com/").Result;
            ...
            foreach (var item in listItems)
            {
                ...
                var time = item.Descendants("span")
                    .FirstOrDefault(x => x.GetAttributeValue("class", "") == "time-label")?.InnerText;
                ...
                if (articleResponse.IsSuccessStatusCode)
                {
                    ...
                    var imageSrc = articleDocument.DocumentNode.SelectSingleNode("//figure/div/div/img")
                    ...
                    if (articleContent != null)
                    {
                        ...
                        SqlCommand command =
                            new SqlCommand(
                                " INSERT INTO ArNews (ArTime, ArTitle, ArContent,Image) VALUES (@time, @title, @content, @imageUrl)",
                                connection);
          ...
          //  MessageBox.Show($"عدد الصفوف التي تم إدخالها  :{count}");
